...
def test_get_data_success(mock_uuid, mock_yaml_load, mock_env):
    with patch("app.routes.SQLBuilder") as mock_builder, \
        ...
        assert response.json()["query_id"] == "12345678-1234-5678-1234-567812345678"
...
def test_get_attributes_success(mock_yaml_load, mock_env):
    with patch("app.routes.SQLBuilder") as mock_builder, \
         patch("app.routes.execute_query") as mock_execute:
        mock_builder_instance = mock_builder.return_value
        mock_builder_instance._validate_columns.return_value = []  # No validation errors
        mock_builder_instance.build_distinct_values_query.return_value = [
            ("uidtype", "VARCHAR", "SELECT DISTINCT uidtype FROM newwpnl WHERE uidtype IS NOT NULL", []),
        ]
        mock_execute.side_effect = [
            [{"uidtype": "value1"}, {"uidtype": "value2"}]  # Distinct values query
        ]
