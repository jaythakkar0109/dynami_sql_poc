...
from app.schemas import GetDataParams, QueryResponse,GetDataRecordsParams
...
@router.post("/rates/risk/{datasource}/get-data-records", response_model=QueryResponse)
async def execute_dynamic_query_records(datasource: str, params: GetDataRecordsParams):
    try:
        sql_builder = SQLBuilder()
        main_query, main_params, count_query, count_params = sql_builder.build_query(params, use_aggregation=False)
        ...
        total_count = 0
        if params.total_count and count_query:
            count_result = execute_query(count_query, count_params, datasource)
            if count_result and len(count_result) > 0:
                total_count = count_result[0]["count"]
        ...
        response = QueryResponse(
            ...
            page_size=params.page_size,
            total_count=total_count if params.total_count else 0,
